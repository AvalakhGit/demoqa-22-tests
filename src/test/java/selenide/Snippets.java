...
public class Snippets {
    ...
    void selectors_examples() {
        ...
        $x("//h1/div").click(); // работает в консоли!!! можно сразу смотреть там элементы
    ...
    void actions_examples() {
        $("").click();
        $("").doubleClick();
        $("").contextClick(); // то же самое что RightClick

        $("").hover();

        $("").setValue("text");
        $("").append("text");
        $("").clear(); //иногда работает неправильно с некоторыми фреймворками.
        $("").setValue(""); // clear
    ...
    void conditions_examples() {
        ...
        $("").shouldHave(cssValue("font-size", "12")); //справа в элементах Computed можно посмотреть атрибуты элемента (cssValue)
        ...
        $("").shouldHave(attributeMatching("name", "[0-9]abc$")); //можно искать элементы с атрибутом начинающимся с конкретных символов, а заканчивающимся на любые
    ...
    void collections_examples() {
        ...
        $$("").shouldHave(itemWithText("Gamma")); // only one text ищем один элемент с конкретным текстом
